  ...
  "View" - {
    ...
    "should include a returns section" - {
      "when there is 1 return pending" - {
        ...
        "that includes details of pending return" - {
          "which has a list of returns due" in {
            document1Return.getElementsByClass(Selectors.body).eachText() must contain(
              "You must send your 1 overdue returns in this order, oldest first:"
            )
          ...
          "which includes a message to send the return" in {
            document1Return.getElementsByClass(Selectors.body).eachText() must contain(
              "You need to send a return to HMRC every quarter, even if you have nothing to report for the period."
            )
          }
        }
      ...
      "when there is more than 1 return pending" - {
        ...
        "that includes details of pending return" - {
          "which has a list of returns due" in {
            document3Return.getElementsByClass(Selectors.body).eachText() must contain(
              "You must send your 3 overdue returns in this order, oldest first:"
            )
    ...
    "should include an account balance section" - {
      ...
      "when the users balance is negative, has no interest, has a DD setup and has a return pending" - {
        ...
        "that includes how to manage direct debit" in {
          ...
          document
            .getElementById("manageExistingDD")
            .text() mustBe "You have set up a Direct Debit to pay the levy. It will not collect any interest and penalties you owe."
        ...
        "that includes details on other ways to pay" in {
          ...
          otherwaystoPay
            .text() mustBe "If you choose to make payments outside of this online account, you will need to take note of your Soft Drinks Levy reference XKSDIL000000022. Find out other ways to pay the levy (opens in a new tab)."
      ...
      "when the users balance is negative, has no interest, has no DD setup and has no returns pending" - {
        ...
        "that includes how to setup direct debit" in {
          ...
          manageSetupDDContent
            .text() mustBe "You can set up a Direct Debit to pay the Soft Drinks Industry Levy. You need to do this at least 3 working days before your return and payment due date."
    ...
    "includes a need help section" - {
      ...
      "that has a link to sdil contact" in {
        ...
        link
          .getElementsByTag("a")
          .attr("href") mustBe "https://www.gov.uk/government/organisations/hm-revenue-customs/contact/soft-drinks-industry-levy"
      }
    }
